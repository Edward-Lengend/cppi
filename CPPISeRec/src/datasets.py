...
class RecWithContrastiveLearningDataset(Dataset):
    def __init__(self, args, user_seq, data_type='train',
        ...
        self.augmentations = {'crop': Crop(self.args, tao=args.tao),
                              'mask': Mask(self.args, gamma=args.gamma),
                              'reorder': Reorder(self.args, beta=args.beta),
    ...
    """
        torch.tensor(user_id, dtype=torch.long),  # user_id是valid_rating_matrix的下标
        torch.tensor(copied_input_ids, dtype=torch.long)#  [0,0,0,12345]补0后的序列,不含原序列后3
        torch.tensor(target_pos, dtype=torch.long),# [0,0,0,2,3,4,5,6]
        torch.tensor(target_neg, dtype=torch.long),# [负样本]
        torch.tensor(answer, dtype=torch.long),# [0]
    """
    ...
    def __getitem__(self, index):
        # 这里的index是用户的标号,从0开始的,和userId不同
        user_id = index
        items = self.user_seq[index]  # 该用户的交互序列
        ...
        if self.data_type == "train":
            input_ids = items[:-3]
            """
                input_ids 是用户历史行为序列,去掉后三个。
            """
            target_pos = items[1:-2]
            """
                target_pos是正样本序列，即用户行为序列中的下一个物品（ground truth）。
            """
            answer = [0]  # no use
            """
                answer这个占位符的作用是为了保证每个训练样本都有一个标签，
                即每个训练样本都需要有一个输出，但是在对比学习中，负样本没有标签，
                因此需要使用一个占位符来占据这个位置。
                另外，这个占位符在训练时并不会对模型的预测结果产生影响，
                因为在计算损失时，这个占位符的贡献会被忽略掉。
            """
        ...
        if self.data_type == "train":
            ...
            total_augmentaion_pairs = nCr(self.n_views, 2)  # 默认返回为1
...
if __name__ == '__main__':
    import argparse
    from utils import get_user_seqs, set_seed, neg_sample, nCr
