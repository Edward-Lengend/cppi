...
class CPPIRecTrainer(Trainer):
    ...
    def __init__(self, model,
                 ...
                 test_dataloader,
                 args):
        super(CPPIRecTrainer, self).__init__(
            model,
            train_dataloader,
            eval_dataloader,
            test_dataloader,
    ...
    def iteration(self, epoch, dataloader, full_sort=True, train=True):
        ...
        if train:
            self.model.train()
            rec_avg_loss = 0.0  # 主任务损失
            cl_individual_avg_losses = [0.0 for i in range(self.total_augmentaion_pairs)]
            # self.total_augmentaion_pairs默认为1对, cl_individual_avg_losses = [0.0]

            cl_sum_avg_loss = 0.0
            joint_avg_loss = 0.0

            print(f"rec dataset length: {len(dataloader)}")  # batch的数量
            ...
            for i, (rec_batch, cl_batches) in rec_cf_data_iter:
                ...
                """
                    input_ids: tensor([0, 0, 727, 2722, 1560]), 
                    target_pos: tensor([0, 0, 2722, 1560, 280], device='cuda:0'), 
                    target_neg: tensor([0, 0, 551, 2332, 3287], device='cuda:0')
                """
                # ---------- recommendation task ---------------#
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    # 检查两个数组的形状是否相同
                    # if rating_pred.shape != self.args.train_matrix.shape:
                    #     print('Error: rating_pred shape does not match train_matrix shape')
                    #     print(type(self.args.train_matrix))
                    #     print(self.args.train_matrix.shape)  # (6040, 3708)
                    #     print(rating_pred.shape)  # (256, 3707)
