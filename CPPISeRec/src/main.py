...
from src.datasets import RecWithContrastiveLearningDataset
from models import OfflineItemSimilarity, SASRecModel
from src.trainers import CPPIRecTrainer
from utils import check_path, set_seed, get_user_seqs, EarlyStopping
...
def main():
    ...
    # print(user_seq)
    """
    user_seq是交互序列，是一个包含物品ID的整数列表。
        例如，如果用户1的交互序列是 [2, 5, 6]，用户2的交互序列是 [1, 3, 7, 2]，
        那么user_seq就是一个包含这两个列表的列表：[[2, 5, 6], [1, 3, 7, 2]]。
    valid_rating_matrix是不含有最后两个交互物品的01交互矩阵
    test_rating_matrix是不含有最后一个交互物品的01交互矩阵
    """

    # ??
    args.item_size = max_item + 1
    # max_item为3706
    print(args.item_size)  # 3707
    # sys.exit(0)
    ...
    # save model
    # checkpoint的机制是：在模型训练的过程中，不断地保存训练结果
    # （包括但不限于EPOCH、模型权重、优化器状态、调度器状态）。
    # 即便模型训练中断，我们也可以基于checkpoint接续训练。
    ...
    # args.training_data_ratio默认为1.0
    ...
    trainer = CPPIRecTrainer(model, train_dataloader, eval_dataloader,
