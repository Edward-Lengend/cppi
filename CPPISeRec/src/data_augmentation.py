...
class Crop(object):
    ...
    def __init__(self, args, tao=0.2):
        self.tao = tao
        self.insert_aug = None
        if args.insert_first:
            self.insert_aug = Insert(item_similarity_model=args.offline_similarity_model, insert_rate=args.insert_rate,
    ...
    def __call__(self, sequence):
        # make a deep copy to avoid original sequence be modified
        copied_sequence = copy.deepcopy(sequence)
        if self.insert_aug is not None:
            copied_sequence = self.insert_aug(copied_sequence)
...
class Reorder(object):
    ...
    def __init__(self, args, beta=0.2):
        self.beta = beta
        self.insert_aug = None
        if args.insert_first:
            self.insert_aug = Insert(item_similarity_model=args.offline_similarity_model, insert_rate=args.insert_rate,
